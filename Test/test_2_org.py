...
# Paths settings
parser.add_argument('--save_path', type= str, default='../Saved/best_sj', help='save path')
#parser.add_argument('--data_path', type=str, required=True, help='path to desired datasets')
parser.add_argument('--data_path', type=str, default='/blue/eel6935/share/GUpsampling/Sparse-Depth-Completion/Data', help='path to desired datasets')
# parser.add_argument('--data_path', type=str, default='../image_gen', help='path to desired datasets')
...
def main():
    ...
    with torch.no_grad():
        #RP changed from "selected_paths" to "val_paths"
        for i, (img, rgb, gt) in tqdm.tqdm(enumerate(zip(dataset.val_paths['lidar_in'],
                                           dataset.val_paths['img'], dataset.val_paths['gt']))):
